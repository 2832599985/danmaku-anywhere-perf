...
- 打开一集要自动补全同批剧集：`src/player/siblingEpisodes.ts` 用"文件名里集数之前的字面量头部"当批次身份（`魔法光源股份有限公司 第 `），同目录+同头部才收、按集数排序、**认不出集数就跳过（不猜）**；片库整个 `Videos` 装一堆番也不会串。`Platform.listVideoFiles()` 在 Tauri 走 fs 插件的 `readDir`（能力里有 `fs:allow-read-dir`），浏览器返回空——扫描是 exe 的便利，不是正确性依赖。视频扩展名只在 `platform/types.ts` 定义一次（选择器/拖拽/扫描共用）。store 的 `insertAfterCurrent()` **移动**已存在的项而不是复制，且永不丢正在播的那一项；开关在 设置 → 播放 →「自动加入同系列剧集」（默认开）。
