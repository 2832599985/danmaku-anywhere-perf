...
import type { BaseMode, EnhancementEffect, PerformanceTier } from '../types'
 ...
 */
const EFFECT_CHAIN_TEMPLATES: Record<
  BaseMode,
  Record<PerformanceTier, string[]>
> = {
  A: {
...
export function resolveEffectChain(
  ...
  return classNames
    .map((className) =>
      AVAILABLE_EFFECTS.find((e) => e.className === className)
    )
...
export function getEffectChainSummary(effects: EnhancementEffect[]): string {
  return (
    effects.map((e) => e.name.split('(')[0].trim()).join(' → ') || 'No effects'
  )
}
